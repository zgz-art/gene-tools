...
# ==================== 新增：AI 评分与风险分析函数 ====================
# 评分提示词（您可以随时修改此字符串来调整评分规则）
SCORE_PROMPT = """你是一位资深 HR 专家，请根据以下简历信息（JSON 格式）和客户岗位技术要求，对该候选人进行综合评分。

评分维度及权重（总分100分）：
1. 学历与专业匹配度（10分）：本科以上学历且专业为计算机相关得高分，否则扣分。
2. IT工作年限（15分）：5年以上满分，3-5年12分，1-3年8分，1年以下4分。
3. 跳槽频率（10分）：平均每份工作超过18个月满分，12-18个月8分，6-12个月5分，低于6个月2分。
4. 技术栈匹配度（20分）：根据技能与岗位要求的关键词重合度评分。
5. 项目经验清晰度（20分）：项目描述是否详细、有量化成果、技术深度。
6. 项目角色及成果（15分）：是否承担核心角色，有具体贡献描述。
7. 稳定性与抗压性（10分）：简历中是否有紧急上线、加班、高强度工作等描述，以及是否有短期工作经历。
