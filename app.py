...
2. 根据文字内容区分判断这张图片属于以下哪种证件类型：
{options}

区分判断要点：
- "身份证正面照片"：直接分析文字内容，有居民身份证、签发机关、有效期限等字样。
- "身份证反面照片"：直接分析文字内容，有姓名、性别、民族、出生日期、住址、公民身份号码等字样。
...
with st.sidebar:
    ...
    model_name = st.selectbox("🤖 选择大模型", ["glm-4-plus", "glm-4-flash", "glm-4.7"], index=0)
    st.markdown("---")
...
st.caption("支持批量上传身份证正反面、毕业证照片、学位证照片、学信网学历/学位备案表截图等，图片命名和上传顺序不影响最终结果，AI 将自动识别图片中的文字并合并到简历中。")
