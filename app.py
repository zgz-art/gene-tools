import os
import sys
import base64  # ===== 新增：用于处理图片base64编码 =====
...
if "uploaded_images" not in st.session_state:
    st.session_state.uploaded_images = None          # 存储上传的图片文件列表
if "image_text" not in st.session_state:
    st.session_state.image_text = None               # 存储从图片中提取的文字
if "last_images_hash" not in st.session_state:
    st.session_state.last_images_hash = None         # 用于检测图片是否变化
...
# ===== 新增：从图片中提取文字（使用智谱GLM-4V）=====
def extract_text_from_images(api_key: str, model: str, image_files) -> str:
    """
    遍历上传的图片文件，调用智谱视觉模型提取每张图片中的文字
    返回合并后的文本
    """
    if not image_files:
        return ""
    client = ZhipuAI(api_key=api_key)
    # 注意：GLM-4V 模型名称可能是 "glm-4v-plus"，但根据你的习惯，保留model参数，用户可能需要手动选
    # 这里强制使用支持视觉的模型，如果原model不是视觉模型则自动切换
    vision_model = "glm-4v-plus"  # 智谱最新视觉模型
    all_texts = []
    for img_file in image_files:
        ...
        try:
            ...
            continue
        # 重置文件指针，以便后续可能的再次读取（实际上提取后不需要再读）
        img_file.seek(0)
    return "\n\n".join(all_texts)
...
# ==================== Streamlit UI ====================
st.title("📄 智能简历填充工具")
st.markdown("上传 PDF 简历、个人资料图片（身份证、毕业证、学位证、学信网截图等）和 Excel 模板，AI 自动提取信息并生成标准格式文件。")
...
with st.expander("📌 使用说明", expanded=True):
    ...
    3. 准备 **Excel 模板**（.xlsx），模板中应包含以下关键字：
       - 基础信息字段（姓名、身份证号...）
       - 精确的 **“本科学历”**、**“研究生学历”** 文字
       - **“工作经历（由近及远，仅限IT相关经历）”**
